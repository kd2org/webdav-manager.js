...
const _ = key => browser.lang in browser.lang_strings && key in browser.lang_strings[browser.lang] ? browser.lang_strings[browser.lang][key] : key;
...
browser.createFilesList = () => {
	...
	items.forEach(item => {
		...
		var html_name = utils.html(item.name.replace(/\.\w{2,5}$/, ''));
		var html_ext = (match = item.name.match(/(\.\w{2,5})$/)) ? utils.html(match[1]) : '';
