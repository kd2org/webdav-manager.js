...
const PREVIEW_EXTENSIONS = /\.(?:png|webp|svg|jpeg|jpg|gif|png|txt|css|js|html?|md|mp4|mkv|webm|ogg|flac|mp3|aac|m4a|avi)$/i;
...
var browser = {
	...
	has_trash: false,
	quota_link_url: null,
	quota_link_title: null,
};

browser.openDialog = (html, confirm_button_label = true, close_button_label = true) => {
	if (confirm_button_label === true) {
		confirm_button_label = _('OK');
	}

	if (close_button_label === true) {
		close_button_label = _('Close');
	}

	var tpl = browser.templates.dialog.replace(/%b/, confirm_button_label ? `<p><input type="submit" value="${confirm_button_label}" /></p>` : '');
	...
	if (close_button_label) {
		d.querySelector('input.close').value = '✖ ' + close_button_label;
	}

	if (a = $('dialog form textarea, dialog form input, dialog form select')) {
		a.focus();
	}
...
browser.replaceDialog = (html, confirm_button_label = true, close_button_label = true) => {
	browser.closeDialog();
	return browser.openDialog(html, confirm_button_label, close_button_label);
};
...
browser.openDeleteDialog = () => {
	if (browser.has_trash) {
		browser.openDialog(browser.templates.trash_dialog, _('Move to trash'), _('Cancel'));
	}
	else {
		browser.openDialog(browser.templates.delete_dialog, _('Delete'), _('Cancel'));
	}
};

browser.getPreference = (name, type, default_value) => {
...
browser.setPreference = (name, value) => {
	browser[name] = value;
...
browser.init = async (url, options) => {
	...
	if (!browser.lang) {
		for (var lang of navigator.languages) {
			...
			if (browser.lang_strings.hasOwnProperty(lang)) {
				browser.lang = lang;
				break;
			}
		}
	...
	// Restore preferences
	browser.sort_order = browser.getPreference('sort_order', 'string', 'name');
	browser.sort_order_desc = browser.getPreference('sort_order_desc', 'bool', false);
	browser.gallery = browser.getPreference('gallery', 'bool', false);
	browser.autosave = browser.getPreference('autosave', 'bool', options.autosave || false);

	browser.search_url = options.browser_url || null;
	browser.search_param_name = options.search_param_name || browser.search_param_name;
	browser.has_trash = options.has_trash || false;
	browser.quota_link_url = options.quota_link_url || null;
	browser.quota_link_title = options.quota_link_title || null;
	...
	document.body.innerHTML = browser.templates.body;

	// Remove any CSS in the header
	css.all('head style:not([webdav])').forEach(e => e.remove());
	...
	document.querySelectorAll('thead td[data-sort] button').forEach(elm => elm.onclick = (e) => {
		...
		browser.setPreference('sort_order', browser.sort_order );
		browser.setPreference('sort_order_desc', browser.sort_order_desc);
		browser.reload();
	});
...
browser.createFilesList = () => {
	...
	// Always sort with directories first
	...
	if (browser.root.url !== browser.webdav_url) {
		...
		// Add breadcrumbs
		var crumbs = browser.root.url.substr(browser.webdav_url.length).replace(/\/$/, '').split('/');
		let url = browser.webdav_url;

		breadcrumbs.innerHTML = `<a href="${url}">${_('My files')}</a> <span>&gt;</span>`;

		for (var crumb of crumbs) {
			url += crumb + '/';
			breadcrumbs.innerHTML += `<a href="${url}">${decodeURIComponent(crumb)}</a> <span>&gt;</span>`;
		}

		breadcrumbs.querySelectorAll('a').forEach(a => a.onclick = () => { browser.open(a.href); return false; });
	}
	else {
	...
	items.forEach(item => {
		...
		if (item.has_thumbnail && browser.enable_thumbnails) {
			item.thumb = '<span class="thumb"><img alt="" src="' + browser.getThumbnailURL(item.path, 150, item.etag) + '" /></span>';
		}
		else {
		...
		var html_name = utils.html(item.name.replace(/\.\w{1,4}$/, ''));
		var html_ext = item.extension ? utils.html('.' + item.extension) : '';
		row.querySelector('a').innerHTML = item.thumb + `<span class="fullname"><span class="name" title="${html_name}">${html_name}</span><span class="ext">${html_ext}</span></span>`;
...
browser.updateSelection = () => {
	css.show('.selection .buttons, .selection .buttons .delete, .selection .buttons .copy, .selection .buttons .move');
	var file_count = 0;
	var dir_count = 0;
	...
	for (var key in browser.selection) {
		...
		if (!file.permissions.includes(PERM_MOVE)) {
			css.hide('.selection .buttons  .move');
		}
...
browser.setRowPermissions = (tr, file) => {
	...
	if (p.includes(PERM_SHARED)) {
		var share_btn = tr.querySelector('.buttons .share');
		share_btn.classList.add('shared');
		share_btn.value = _('Shared');
		share_btn.title = _('Open share options');
	}
};
...
browser.setRowActions = (tr, file) => {
	...
	$$('.buttons .options').onclick = () => $$('.buttons div').classList.toggle('open');
	...
	$$('.buttons .rename').onclick = () => {
		browser.openDialog(browser.templates.rename_dialog, _('Rename'), _('Cancel'));
		let t = $('input[name=rename]');
		t.value = file.name;
		t.focus();
		t.selectionStart = 0;
		t.selectionEnd = file.name.lastIndexOf('.');
		$('#dialog_form').onsubmit = () => {
	...
	$$('.buttons .share').onclick = () => {
		browser.openDialog(browser.templates.share_dialog, _('Create share link'), _('Cancel'));
		var f = $('#dialog_form');

		f.onsubmit = () => {
			browser.createShare(file, f.permissions.value, f.expiry.value, f.password.value);
			return false;
		};
	};
...
browser.openPreview = (file) => {
	...
	if (file.mime.match(/^image\//)) {
		...
		browser.openDialog(`<audio controls="true" autoplay="false" src="${file.url}" />`, false);
	}
	else if (file.mime.match(/^video\//)) {
		browser.openDialog(`<video controls="true" autoplay="false" src="${file.url}" />`, false);
	}
...
browser.editTextFile = (file) => {
	req('GET', file.url).then((r) => r.text().then((t) => {
		...
		if (window.prismEditor) {
			var language = null;

			if (file.name.match(/\.html?$/i)) {
				language = 'html';
			}
			else if (file.name.match(/\.(?:js|json)$/i)) {
				language = 'js';
			}
			...
			const editor = prismEditor('.editor', {
				language,
				value: txt.value,
				wordWrap: true,
				insertSpaces: false,
			...
			editor.textarea.name = txt.name;
			txt.remove();
			txt = editor.textarea;

			window.setTimeout(() => txt.focus(), 500);
		}
		...
		autosave.checked = browser.autosave;
		autosave.onchange = () => {
			browser.setPreference('autosave', autosave.checked);
		};
...
browser.pasteTo = function (file, action) {
	...
	if (action === 'move' && browser.root.url === utils.dirname(file.url)) {
		alert(_('Cannot move to the same folder'));
		return;
	}
...
browser.toggleGalleryView = () => {
	css.hide('.toolbar .settings .list, .toolbar .settings .gallery');

	if (browser.gallery) {
		css.show('.toolbar .settings .list');
	}
	else {
		css.show('.toolbar .settings .gallery')
	}

	document.querySelector('body').classList.toggle('gallery', browser.gallery);
	document.querySelector('body').classList.toggle('list', !browser.gallery);
};
...
browser.createToolbar = () => {
	...
	$('.selection .buttons .move').onclick = () => browser.createPasteSelection('move');
	...
	$('.toolbar .settings .list').onclick = () => { browser.setPreference('gallery', false); browser.toggleGalleryView(); };
	$('.toolbar .settings .gallery').onclick = () => { browser.setPreference('gallery', true); browser.toggleGalleryView(); };

	browser.toggleGalleryView();

	// Hide stuff that can only be used if permissions allow
	css.hide('.toolbar .create, .selection .buttons .copy, .selection .buttons .move, .selection .buttons .delete');
	css.hide('.toolbar .menu, .toolbar .menu .wopi, .toolbar .paste');
	...
	if (wopi.extensions) {
		...
		css.onclick('.toolbar .menu .wopi input', (ev, btn) => {
			toggle_menu();
			browser.openDialog(browser.templates.mkfile_dialog, _('Create this file'), _('Cancel'));
			...
			$('#dialog_form').onsubmit = () => {
				...
				fetch('data:application/octet-stream;base64,' + wopi.OPENDOCUMENT_TEMPLATES[ext]).then(r => r.blob()).then(r => {
	...
	$('.mkdir').onclick = () => {
		toggle_menu();
		browser.openDialog(browser.templates.mkdir_dialog, _('Create this folder'), _('Cancel'));
		$('#dialog_form').onsubmit = () => {
...
browser.updateQuota = () => {
	...
	if (null === browser.root.used_quota || null === browser.root.available_quota) {
		return;
	}

	var total = browser.root.used_quota + browser.root.available_quota;
	var percent_used = Math.min(100, Math.ceil((browser.root.used_quota / total) * 100));
	var a = $('.quota a');

	if (browser.quota_link_url) {
		a.href = browser.quota_link_url;
		a.title = browser.quota_link_title;
		a.target = '_blank';
	}

	a.setAttribute('style', '--percent: ' + percent_used);

	a.querySelector('.free').innerText = sprintf(_('%s free'), utils.formatBytes(browser.root.available_quota));
	a.querySelector('.details').innerText = sprintf(
		_('%s used (%s of %s)'),
		percent_used + '%',
		utils.formatBytes(browser.root.used_quota),
...
browser.pasteFile = (e) => {
	...
	for (var i = 0; i < items.length; i++) {
		if (items[i].kind === 'file' || IMAGE_MIME_REGEX.test(items[i].type)) {
			...
			browser.openDialog(browser.templates.paste_upload_dialog, _('OK'), _('Cancel'));
...
browser.createShare = (file, permissions, expiry, password) => {
	var date = new Date;
	date.setDate(date.getDate() + expiry*60*60);
	...
	if (file.is_dir) {
		alert('Cannot share a folder for now'); // TODO
		return false;
	}

	var url = browser.server_url + 'ocs/v2.php/apps/files_sharing/api/v1/shares';

	dav.send('POST', url, new URLSearchParams(data), {'OCS-APIRequest': 'true'})
		.then(r => {
			if (!r.ok) {
				browser.closeDialog();
				alert(_('Failed to create link'));
				throw r;
			}
		...
		.then(xml => {
			browser.reload();
			browser.replaceDialog(browser.templates.share_link_dialog, null);
			var i = $('#dialog_form fieldset input[type=text]');
			var btn = $('#dialog_form fieldset input[type=button]');
			i.value = xml.querySelector('data url').textContent;
			i.size = i.value.length;
			i.onclick = btn.onclick = () => {
				i.focus();
				i.select();
...
browser.getThumbnailURL = (path, size=150, etag) => `${browser.server_url}index.php/apps/files/api/v1/thumbnail/${size}/${size}/${path}`;

browser.populateTemplates = () => {
	browser.templates.mkdir_dialog = `<input type="text" name="mkdir" placeholder="${_('Folder name')}" />`;
	...
	browser.templates.paste_upload_dialog = `<h3>${_('Upload this file?')}</h3><input type="text" name="paste_name" placeholder="${_('New file name')}" />`;
	browser.templates.edit_dialog = `<div class="editor"><textarea name="edit" cols="70" rows="30"></textarea></div>`;
	browser.templates.markdown_dialog = `<div class="dual"><div class="editor"><textarea name="edit" cols="70" rows="30"></textarea></div><div class="md_preview"></div></div>`;
	browser.templates.delete_dialog = `<h3>${_('Confirm delete?')}</h3>`;
	browser.templates.trash_dialog = `<h3>${_('Move to trash?')}</h3>`;
	browser.templates.share_link_dialog = `<fieldset><legend>${_('Send this link to your contacts')}</legend>
		<p class="url"><input type="text" readonly /><input type="button" value="${_('Copy')}" /></p></fieldset>`;

	// permission = (int) 1 = read; 2 = update; 4 = create; 8 = delete; 16 = share; 31 = all
	browser.templates.share_dialog = `<fieldset>
		<legend>${_('Share options')}</legend>
		<p class="help">${_('A public share link will be created, allowing access to these files without a login.')}</p>
		<dl class="permissions">
			<dt><label for="f_permissions">${_('What can people do with this link?')}</label></dt>
			<dd>
				<select name="permissions" id="f_permissions">
					<option value="1">${_('Only view and download')}</option>
					<option value="2">${_('Edit, view and download')}</option>
				</select>
			</dd>
		</dl>
		<dl class="duration">
			<dt><label for="f_expiry">${_('When will this link expire?')}</label></dt>
			<dd>
				<select name="expiry" id="f_expiry">
					<option value="1">${_('One hour')}</option>
					<option value="24">${_('One day')}</option>
					<option value="${24*7}">${_('One week')}</option>
					<option value="${24*31}" selected="selected">${_('One month')}</option>
					<option value="${24*90}">${_('3 months')}</option>
					<option value="${24*180}">${_('6 months')}</option>
					<option value="${24*365}">${_('One year')}</option>
					<option value="0">${_('Never')}</option>
				</select>
			</dd>
		</dl>
		<dl class="password">
			<dt><label for="f_password">${_('Ask for a password?')}</label> <small>${_('(optional)')}</dt>
			<dd><input type="password" name="password" id="f_password" /></dd>
			<dd class="help">${_('If supplied, people will need to enter this password to access the shared files.')}</dd>
		</dl>
		</fieldset>`;

	browser.templates.dialog = `<dialog open><p class="close"><input type="button" value="&#x2716; ${_('Close')}" class="close" /></p><form id="dialog_form"><div>%s</div>%b</form></dialog>`;
		...
		<div class="toolbar">
			<div class="selection" style="display: none">
				<div class="buttons">
					...
					<input type="button" class="icon zip" value="ZIP" title="${_('Download as ZIP')}" />
					<input type="button" class="icon delete" value="${_('Delete')}" />
					<input type="button" class="icon move" value="${_('Move')}" />
			...
			<div class="settings">
				<input class="icon list" type="button" value="${_('View as list')}" />
				<input class="icon gallery" type="button" value="${_('View as gallery')}" />
			</div>
			...
			<form method="get" action="" target="_blank" class="search" style="display: none">
				<input type="text" placeholder="${_('Search in files')}" />
				<input type="submit" class="icon search" value="${_('Search')}" />
			</form>
			<div class="create">
				...
				<div class="menu">
					<input class="icon mkdir" type="button" value="${_('Folder')}" />
	...
	browser.templates.file_row = `
		...
		<td class="date"></td>
		<td class="buttons">
			<input class="icon options" type="button" value="${_('Options')}" title="${_('Options')}" />
			<div>
				...
				<input class="icon share" type="button" value="${_('Share')}" title="${_('Share')}" />
