	...
	items.forEach(item => {
		...
		*/
		...
		if (item.has_thumbnail && browser.enable_thumbnails) {
			item.thumb = '<img alt="" src="' + browser.getThumbnailURL(item.path, 150, item.etag) + '" />';
		}
		else {
		...
		row.querySelector('a').innerHTML = item.thumb + '<span class="name">' + utils.html(item.name) + '</span>';
