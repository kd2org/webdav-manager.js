const _ = (key, ...argv) => {
	if (browser.lang in browser.lang_strings && key in browser.lang_strings[browser.lang]) {
		key = browser.lang_strings[browser.lang][key];
	}
...
const sprintf = (str, ...argv) => !argv.length ? str :
    sprintf(str = str.replace(/%[ds]/, argv.shift()), ...argv);
